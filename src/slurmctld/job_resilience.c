...
static int _regrow_with_node(job_record_t *job_ptr, node_record_t *node_ptr)
{
	...
	job_desc->work_dir = xstrdup(detail_ptr->work_dir ?
				     detail_ptr->work_dir : "/");
	 ...
	 */
	rc = job_allocate(job_desc, 1, false, NULL, true, 0, false,
			  &helper_ptr, &err_msg, SLURM_PROTOCOL_VERSION);
...
static int _foreach_collect_shrunk(void *x, void *arg)
{
	...
	if ((job_ptr->bit_flags & ADAPTIVE_RESILIENCE) &&
	    job_ptr->resilience_shrink_time && job_ptr->resilience_orig_bitmap &&
	    IS_JOB_RUNNING(job_ptr) && !IS_JOB_CONFIGURING(job_ptr) &&
	    (job_ptr->node_cnt < job_ptr->resilience_orig_node_cnt))
